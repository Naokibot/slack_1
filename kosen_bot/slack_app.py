...
def _reminder_modal(
    ...
    *,
    ...
    date_element: dict = {
        "type": "datepicker",
        "action_id": "value",
        "placeholder": {"type": "plain_text", "text": "日付"},
    }
    if day:
...
def register_handlers(
    ...
    def handle_kosen(ack, command, respond):
        ...
        if parts[0] == "set-date" and len(parts) == 2:
            ...
            respond(
                f"✅ 高専入試日を {format_japanese_date(new_date)} に変更しました。"
            )
            return
    ...
    def handle_reminder(ack, command, client, respond):
        ...
        if text in {"today", "tomorrow"}:
            ...
            lines = [f"📅 *{label}の予定*", ""]
            lines.extend(
                f"{item.scheduled_at:%H:%M} {item.title}  `#{item.id}`"
                for item in items
            )
            respond("\n".join(lines))
            return
    ...
    def handle_reminder_edit(ack, body, client):
        try:
            ...
            ack(
                response_action="errors",
                errors={"title": f"入力を確認してください: {exc}"},
            )
            return

        if not db.update_reminder(
            reminder_id,
            item.title,
            item.scheduled_at,
        ):
        ...
        client.chat_postEphemeral(
            channel=channel_id,
            user=user_id,
            text=f"✅ リマインダー #{reminder_id} を更新しました。",
        )
